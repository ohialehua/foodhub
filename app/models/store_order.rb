class StoreOrder < ApplicationRecord
  ...
  def create_store_notification_order(current_enduser, store_order_id, store_id)
    notification = StoreNotification.where(["enduser_id = ? and store_id = ? and store_order_id = ? and action = ? ",current_enduser.id, store_id, store_order_id, 'order'])
  ...
  def create_public_notification_complete(current_store, store_order_id, enduser_id)
    ...
    #検索が複数条件で複雑化しているのでプレースホルダを記述(SQLインジェクション対策)
    if notification.blank?
      public_notification = current_store.public_notifications.new(
        store_order_id: store_order_id,
